    ...
    internal class AntiPiracy
    {
        public static bool IsInvalid(string path)
        {
            ...
            return
                ...
                Directory.EnumerateFiles(path, "*.url").Any() ||
                Directory.EnumerateFiles(path, "*.ini").Any() ||
                Directory.EnumerateFiles(dataPlugins, "*.ini", SearchOption.AllDirectories).Any() ||
                new DirectoryInfo(dataPlugins).EnumerateFiles("*steam_api64.dll", SearchOption.AllDirectories).FirstOrDefault()?.Length >= 300 * 1024;
        }
